object FCapturaCargo: TFCapturaCargo
  Left = 305
  Top = 112
  BorderStyle = bsSingle
  Caption = 'Registro de Cargos'
  ClientHeight = 611
  ClientWidth = 1186
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -10
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1186
    Height = 611
    Align = alClient
    TabOrder = 0
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 1184
      Height = 168
      Align = alTop
      TabOrder = 0
      object Label2: TLabel
        Left = 485
        Top = 8
        Width = 85
        Height = 13
        Caption = 'Numero Cargo:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 408
        Top = 34
        Width = 162
        Height = 13
        Caption = 'Fecha del Cargo (Evento/s):'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 8
        Top = 139
        Width = 65
        Height = 13
        Caption = 'Ar'#233'a Prod.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label13: TLabel
        Left = 8
        Top = 4
        Width = 431
        Height = 27
        Caption = 'CARGOS A LA CUENTA DEL PACIENTE'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -24
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object Label18: TLabel
        Left = 423
        Top = 139
        Width = 121
        Height = 13
        Caption = 'Tipo Admisi'#243'n:(RIPS)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DateTimePicker1: TDateTimePicker
        Left = 575
        Top = 27
        Width = 177
        Height = 21
        Date = 38547.518292789350000000
        Time = 38547.518292789350000000
        TabOrder = 1
        TabStop = False
      end
      object Edit1: TEdit
        Left = 576
        Top = 5
        Width = 177
        Height = 21
        TabStop = False
        Color = cl3DLight
        ReadOnly = True
        TabOrder = 0
      end
      object GroupBox4: TGroupBox
        Left = 8
        Top = 50
        Width = 745
        Height = 83
        Caption = 'Ingreso:'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        object Label4: TLabel
          Left = 8
          Top = 40
          Width = 55
          Height = 13
          Caption = 'Paciente:'
        end
        object Label1: TLabel
          Left = 484
          Top = 63
          Width = 34
          Height = 13
          Caption = 'Edad:'
        end
        object Label7: TLabel
          Left = 282
          Top = 64
          Width = 56
          Height = 13
          Caption = 'Cama No:'
        end
        object Label12: TLabel
          Left = 8
          Top = 63
          Width = 86
          Height = 13
          Caption = 'Fecha Ingreso:'
        end
        object Label17: TLabel
          Left = 218
          Top = 17
          Width = 76
          Height = 13
          Caption = 'Aseguradora:'
        end
        object Label6: TLabel
          Left = 483
          Top = 17
          Width = 73
          Height = 13
          Caption = 'No Contrato:'
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 8
          Top = 13
          Width = 206
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'NO_ADMISION'
          ListField = 'NO_ADMISION'
          ListSource = DataSource1
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          OnExit = DBLookupComboBox1Exit
        end
        object DBEdit1: TDBEdit
          Left = 99
          Top = 59
          Width = 177
          Height = 21
          TabStop = False
          DataField = 'FEC_ADMISION'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 8
        end
        object DBEdit2: TDBEdit
          Left = 344
          Top = 59
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'NO_CAMA'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 9
        end
        object DBEdit7: TDBEdit
          Left = 216
          Top = 37
          Width = 129
          Height = 21
          TabStop = False
          DataField = 'PRIMER_APELLIDO'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 4
        end
        object DBEdit8: TDBEdit
          Left = 344
          Top = 37
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'SEGUNDO_APELLIDO'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 5
        end
        object DBEdit9: TDBEdit
          Left = 480
          Top = 37
          Width = 129
          Height = 21
          TabStop = False
          DataField = 'PRIMER_NOMBRE'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 6
        end
        object DBEdit10: TDBEdit
          Left = 608
          Top = 37
          Width = 129
          Height = 21
          TabStop = False
          DataField = 'SEGUNDO_NOMBRE'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 7
        end
        object DBLookupComboBox10: TDBLookupComboBox
          Left = 64
          Top = 37
          Width = 150
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'key2'
          ListField = 'NR_IDENT_USUAR;TP_IDENT_USUAR'
          ListSource = DataSource1
          ParentFont = False
          ReadOnly = True
          TabOrder = 3
          OnExit = DBLookupComboBox10Exit
        end
        object DBEdit4: TDBEdit
          Left = 293
          Top = 10
          Width = 184
          Height = 21
          TabStop = False
          DataField = 'ASEGURADORA'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
        end
        object DBEdit11: TDBEdit
          Left = 560
          Top = 13
          Width = 177
          Height = 21
          TabStop = False
          DataField = 'NO_CONTRATO'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 2
        end
        object EditEdad: TJvEdit
          Left = 524
          Top = 59
          Width = 213
          Height = 21
          TabStop = False
          ClipboardCommands = [caCopy]
          CharCase = ecUpperCase
          Color = cl3DLight
          ReadOnly = True
          TabOrder = 10
          Text = ''
        end
      end
      object Edit2: TEdit
        Left = 107
        Top = 136
        Width = 310
        Height = 21
        Enabled = False
        TabOrder = 3
        Text = 'Edit2'
      end
      object DBLookupComboBox11: TDBLookupComboBox
        Left = 550
        Top = 137
        Width = 203
        Height = 21
        Color = clYellow
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        KeyField = 'CD_TIPO_ADMISION'
        ListField = 'DESCRIPCION'
        ListSource = DataSource7
        ParentFont = False
        TabOrder = 4
        TabStop = False
        OnEnter = DBLookupComboBox11Enter
        OnExit = DBLookupComboBox1Exit
      end
      object BitBtn8: TBitBtn
        Left = 758
        Top = 3
        Width = 166
        Height = 58
        Caption = 'Imprimir Hoja Cargo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FBE999FBE999FBE999FBE999FBE999FBE999FBE999FBE999FBE9}
        ParentFont = False
        TabOrder = 5
        TabStop = False
        OnClick = BitBtn8Click
      end
      object BitBtn9: TBitBtn
        Left = 757
        Top = 97
        Width = 166
        Height = 58
        Caption = 'Modificar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FBE999FBE999FBE999FBE999FBE999FBE999FBE999FBE999FBE9}
        ParentFont = False
        TabOrder = 6
        TabStop = False
        OnClick = BitBtn9Click
      end
      object pnl: TPanel
        Left = 800
        Top = 1
        Width = 383
        Height = 166
        Align = alRight
        Enabled = False
        TabOrder = 7
        object Sw1: TJvSwitch
          Left = 7
          Top = -4
          Width = 187
          Height = 45
          Caption = 'Valida Existencias.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          GlyphOff.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          GlyphOn.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          ParentFont = False
          ToggleKey = 0
          TabOrder = 0
          TextPosition = tpRight
          Data = {03}
        end
        object Sw2: TJvSwitch
          Left = 7
          Top = 36
          Width = 187
          Height = 45
          Caption = 'Productos x Area.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          GlyphOff.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          GlyphOn.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          ParentFont = False
          ToggleKey = 0
          TabOrder = 1
          TextPosition = tpRight
          Data = {03}
        end
        object Sw3: TJvSwitch
          Left = 7
          Top = 76
          Width = 187
          Height = 45
          Caption = 'Valida Homologaci'#243'n.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          GlyphOff.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          GlyphOn.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          ParentFont = False
          ToggleKey = 0
          TabOrder = 2
          TextPosition = tpRight
          Data = {03}
        end
        object Sw4: TJvSwitch
          Left = 7
          Top = 117
          Width = 187
          Height = 45
          Caption = 'Convenio como Dscto.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          GlyphOff.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          GlyphOn.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          ParentFont = False
          ToggleKey = 0
          TabOrder = 3
          TextPosition = tpRight
          Data = {03}
        end
        object Sw5: TJvSwitch
          Left = 191
          Top = 0
          Width = 187
          Height = 45
          Caption = 'Servicios x Tipo Adm.'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          GlyphOff.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          GlyphOn.Data = {
            
            DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD}
          ParentFont = False
          ToggleKey = 0
          TabOrder = 4
          TextPosition = tpRight
          Data = {03}
        end
      end
    end
    object PageControl1: TPageControl
      Left = 1
      Top = 169
      Width = 1184
      Height = 441
      ActivePage = TabSheet1
      Align = alClient
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -10
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      TabStop = False
      OnChange = PageControl1Change
      object TabSheet1: TTabSheet
        Caption = 'Detalle del Cargo:'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object GroupBox1: TGroupBox
          Left = 0
          Top = 0
          Width = 1176
          Height = 413
          Align = alClient
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 0
          object StringGrid1: TStringGrid
            Left = 2
            Top = 15
            Width = 1172
            Height = 206
            Cursor = crHandPoint
            TabStop = False
            Align = alClient
            ColCount = 15
            Ctl3D = True
            RowCount = 2
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -10
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goColSizing, goRowSelect]
            ParentCtl3D = False
            ParentFont = False
            PopupMenu = PopupMenu2
            TabOrder = 0
            OnClick = StringGrid1Click
            RowHeights = (
              24
              24)
          end
          object GroupBox3: TGroupBox
            Left = 2
            Top = 221
            Width = 1172
            Height = 190
            Align = alBottom
            Caption = 'Items:'
            TabOrder = 1
            object Label8: TLabel
              Left = 8
              Top = 20
              Width = 63
              Height = 13
              Caption = 'Prod/Serv:'
            end
            object Label9: TLabel
              Left = 348
              Top = 68
              Width = 31
              Height = 13
              Caption = 'Cant:'
              Visible = False
            end
            object Label10: TLabel
              Left = 431
              Top = 68
              Width = 69
              Height = 13
              Caption = 'Vr. Unitario:'
              Visible = False
            end
            object Label11: TLabel
              Left = 580
              Top = 114
              Width = 79
              Height = 13
              Caption = 'Vr. Total Liq.:'
              Visible = False
            end
            object Label14: TLabel
              Left = 111
              Top = 44
              Width = 38
              Height = 13
              Caption = 'Tarifa:'
              Visible = False
            end
            object Label15: TLabel
              Left = 8
              Top = 44
              Width = 5
              Height = 13
              Visible = False
            end
            object Label16: TLabel
              Left = 72
              Top = 44
              Width = 33
              Height = 13
              AutoSize = False
              Visible = False
            end
            object Label21: TLabel
              Left = 8
              Top = 67
              Width = 30
              Height = 13
              Caption = 'Lote:'
              Visible = False
            end
            object Label23: TLabel
              Left = 8
              Top = 150
              Width = 52
              Height = 13
              Caption = 'Paquete:'
              Visible = False
            end
            object Shape1: TShape
              Left = 8
              Top = 142
              Width = 745
              Height = 1
            end
            object Label24: TLabel
              Left = 551
              Top = 152
              Width = 31
              Height = 13
              Caption = 'Cant:'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = [fsBold]
              ParentFont = False
              Visible = False
            end
            object Label25: TLabel
              Left = 270
              Top = 68
              Width = 32
              Height = 13
              Caption = 'Exist:'
              Visible = False
            end
            object Label22: TLabel
              Left = 617
              Top = 90
              Width = 43
              Height = 13
              Caption = 'Liquid.:'
            end
            object Label26: TLabel
              Left = 625
              Top = 66
              Width = 35
              Height = 13
              Caption = '% liq.:'
            end
            object Label19: TLabel
              Left = 431
              Top = 92
              Width = 69
              Height = 13
              Caption = 'Costo Prom:'
              Visible = False
            end
            object Label28: TLabel
              Left = 40
              Top = 68
              Width = 5
              Height = 13
            end
            object Label27: TLabel
              Left = 610
              Top = 152
              Width = 10
              Height = 13
              Caption = '%'
            end
            object DBLookupComboBox3: TDBLookupComboBox
              Left = 151
              Top = 17
              Width = 113
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'CD_PRODUCTO'
              ListField = 'CD_PRODUCTO'
              ListSource = DataSource3
              ParentFont = False
              TabOrder = 18
              OnExit = DBLookupComboBox3Exit
            end
            object DBLookupComboBox2: TDBLookupComboBox
              Left = 151
              Top = 17
              Width = 113
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'CODIGO'
              ListField = 'CODIGO'
              ListSource = DataSource4
              ParentFont = False
              TabOrder = 1
              OnExit = DBLookupComboBox2Exit
            end
            object MaskEdit1: TMaskEdit
              Left = 380
              Top = 65
              Width = 49
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 8
              Text = ''
              Visible = False
              OnChange = MaskEdit2Change
              OnClick = MaskEdit1Click
              OnExit = MaskEdit1Exit
              OnKeyPress = MaskEdit1KeyPress
            end
            object MaskEdit2: TMaskEdit
              Left = 502
              Top = 65
              Width = 81
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 9
              Text = ''
              Visible = False
              OnChange = MaskEdit2Change
              OnExit = MaskEdit2Exit
              OnKeyPress = MaskEdit2KeyPress
            end
            object MaskEdit3: TMaskEdit
              Left = 662
              Top = 111
              Width = 89
              Height = 21
              TabStop = False
              Color = clMenuBar
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 11
              Text = ''
              Visible = False
            end
            object DBLookupComboBox4: TDBLookupComboBox
              Left = 270
              Top = 17
              Width = 379
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'CD_PRODUCTO'
              ListField = 'DESCRIPCION'
              ListSource = DataSource3
              ParentFont = False
              TabOrder = 19
              OnExit = DBLookupComboBox3Exit
            end
            object ComboBox1: TComboBox
              Left = 72
              Top = 17
              Width = 77
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              OnChange = ComboBox1Change
              OnExit = ComboBox1Exit
              Items.Strings = (
                'P'
                'S'
                'PAQUETE')
            end
            object DBLookupComboBox6: TDBLookupComboBox
              Left = 270
              Top = 17
              Width = 398
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'CODIGO'
              ListField = 'DESCRIPCION'
              ListSource = DataSource4
              ParentFont = False
              TabOrder = 2
              TabStop = False
              OnExit = DBLookupComboBox2Exit
            end
            object DBLookupComboBox9: TDBLookupComboBox
              Left = 150
              Top = 40
              Width = 600
              Height = 21
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'Cod'
              ListField = 'Descr'
              ListSource = DataSource2
              ParentFont = False
              TabOrder = 5
              TabStop = False
              Visible = False
              OnExit = DBLookupComboBox9Exit
            end
            object BitBtn2: TBitBtn
              Left = 755
              Top = 11
              Width = 159
              Height = 54
              Caption = 'Limpiar Grilla'
              Glyph.Data = {
                
                FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              TabOrder = 15
              TabStop = False
              OnClick = BitBtn2Click
            end
            object BitBtn3: TBitBtn
              Left = 755
              Top = 67
              Width = 159
              Height = 54
              Caption = 'Adicionar a la Grilla'
              Glyph.Data = {
                
                FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              TabOrder = 16
              TabStop = False
              OnClick = BitBtn3Click
            end
            object BitBtn4: TBitBtn
              Left = 755
              Top = 123
              Width = 159
              Height = 62
              Caption = 'Grabar el Cargo'
              Glyph.Data = {
                
                FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
              TabOrder = 17
              TabStop = False
              OnClick = BitBtn4Click
            end
            object BitBtn5: TBitBtn
              Left = 583
              Top = 65
              Width = 25
              Height = 21
              Glyph.Data = {
                
                00333377737FFFFF773333303300000003333337337777777333}
              NumGlyphs = 2
              TabOrder = 20
              TabStop = False
              OnClick = BitBtn5Click
            end
            object DBLookupComboBox12: TDBLookupComboBox
              Left = 64
              Top = 64
              Width = 201
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'llave'
              ListField = 'llave;Cantidad'
              ListSource = DataSourceSVencim
              ParentFont = False
              TabOrder = 6
              TabStop = False
              Visible = False
            end
            object BitBtn6: TBitBtn
              Left = 725
              Top = 16
              Width = 25
              Height = 22
              Hint = 'Revisar el Convenio con la Aseguradora'
              Glyph.Data = {
                
                3333333333333333333333333333333333333333333333333333}
              NumGlyphs = 2
              ParentShowHint = False
              ShowHint = True
              TabOrder = 3
              TabStop = False
              Visible = False
              OnClick = BitBtn6Click
            end
            object BitBtn7: TBitBtn
              Left = 674
              Top = 16
              Width = 25
              Height = 22
              Hint = 'Crear o moficiar el producto o servicio'
              Glyph.Data = {
                
                3000333333333333377733333333333333333333333333333333}
              NumGlyphs = 2
              ParentShowHint = False
              ShowHint = True
              TabOrder = 4
              TabStop = False
              Visible = False
              OnClick = BitBtn7Click
            end
            object DBLookupComboBox13: TDBLookupComboBox
              Left = 64
              Top = 147
              Width = 481
              Height = 21
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              KeyField = 'Codigo'
              ListField = 'Descripcion'
              ListSource = DataSourceEncAgrupa
              ParentFont = False
              TabOrder = 12
              TabStop = False
              Visible = False
            end
            object DBEdit5: TDBEdit
              Left = 662
              Top = 149
              Width = 87
              Height = 21
              TabStop = False
              Color = cl3DLight
              DataField = 'Liq'
              DataSource = DataSourceEncAgrupa
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              ReadOnly = True
              TabOrder = 14
              Visible = False
            end
            object MaskEdit4: TMaskEdit
              Left = 581
              Top = 149
              Width = 27
              Height = 21
              TabStop = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 13
              Text = '1'
              Visible = False
              OnKeyPress = MaskEdit1KeyPress
            end
            object DBEdit6: TDBEdit
              Left = 301
              Top = 64
              Width = 41
              Height = 21
              TabStop = False
              Color = clMenuBar
              DataField = 'Cantidad'
              DataSource = DataSourceSVencim
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 7
              Visible = False
            end
            object MaskEdit12: TMaskEdit
              Left = 662
              Top = 87
              Width = 89
              Height = 21
              TabStop = False
              Color = clMenuBar
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 10
              Text = ''
            end
            object ChkIncluir: TCheckBox
              Left = 8
              Top = 121
              Width = 273
              Height = 17
              TabStop = False
              Caption = 'Incluir el producto al paquete seleccionado.'
              TabOrder = 21
            end
            object MaskEdit5: TMaskEdit
              Left = 662
              Top = 63
              Width = 89
              Height = 21
              TabStop = False
              Color = clMenuBar
              Enabled = False
              TabOrder = 22
              Text = ''
            end
            object MaskEdit6: TMaskEdit
              Left = 502
              Top = 89
              Width = 81
              Height = 21
              TabStop = False
              Color = cl3DLight
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              TabOrder = 23
              Text = ''
              Visible = False
              OnChange = MaskEdit2Change
              OnExit = MaskEdit2Exit
              OnKeyPress = MaskEdit2KeyPress
            end
            object BitBtn10: TBitBtn
              Left = 700
              Top = 16
              Width = 25
              Height = 22
              Hint = 'Homologaci'#243'n del servicio'
              Glyph.Data = {
                
                00333377737FFFFF773333303300000003333337337777777333}
              NumGlyphs = 2
              ParentShowHint = False
              ShowHint = True
              TabOrder = 24
              TabStop = False
              OnClick = BitBtn10Click
            end
            object DBEdit12: TDBEdit
              Left = 620
              Top = 149
              Width = 41
              Height = 21
              TabStop = False
              Color = cl3DLight
              DataField = 'Porcentaje'
              DataSource = DataSourceEncAgrupa
              Enabled = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -10
              Font.Name = 'MS Sans Serif'
              Font.Style = []
              ParentFont = False
              ReadOnly = True
              TabOrder = 25
              Visible = False
            end
          end
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Cargos ya Realizados:'
        ImageIndex = 1
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object DBGrid1: TDBGrid
          Left = 0
          Top = 0
          Width = 1176
          Height = 413
          Align = alClient
          Color = cl3DLight
          DataSource = DataSource5
          DrawingStyle = gdsGradient
          PopupMenu = PopupMenu
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -10
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = [fsBold]
        end
      end
    end
  end
  object DataSource3: TDataSource
    AutoEdit = False
    DataSet = ADOQueryProd
    OnDataChange = DataSource3DataChange
    Left = 697
    Top = 265
  end
  object ADOQuery2: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * '
      'FROM CREncCargos'
      'WHERE No_Cargo = 0  AND Estado is NULL')
    Left = 41
    Top = 257
  end
  object ADOQuery3: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * '
      'FROM CRDetCargos'
      'WHERE No_Cargo = 0')
    Left = 73
    Top = 257
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    OnDataChange = DataSource1DataChange
    Left = 9
    Top = 1
  end
  object ADOQuery1: TADOQuery
    Connection = DataModule1.ADOConnection1
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'adm'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT *,Primer_Apellido+'#39' '#39'+Segundo_Apellido+'#39' '#39'+Primer_Nombre+' +
        #39' '#39'+Segundo_Nombre as Nom,SAPacientes.nr_ident_usuar+SAPacientes' +
        '.tp_ident_usuar+no_admision as key2 '
      'FROM SAAdmisiones,SAPacientes '
      
        'WHERE (SAAdmisiones.Tp_Ident_Usuar = SAPacientes.Tp_Ident_Usuar ' +
        'AND'
      
        '               SAAdmisiones.Nr_Ident_Usuar = SAPacientes.Nr_Iden' +
        't_Usuar) AND No_Admision = :adm'
      'ORDER BY No_Admision')
    Left = 73
    Top = 1
  end
  object DataSource4: TDataSource
    AutoEdit = False
    DataSet = ADOQueryServ
    Enabled = False
    OnDataChange = DataSource4DataChange
    Left = 697
    Top = 321
  end
  object DataSource2: TDataSource
    DataSet = ADOQueryCrucServHomo
    Left = 32
    Top = 320
  end
  object ADOQueryCrucServHomo: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Tbl'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 3
        Value = Null
      end
      item
        Name = 'Cod'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 15
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT SAServicios.Tabla+'#39'-'#39'+SAServicios.Codigo+'#39'-'#39'+SATarifarios' +
        '.Tabla+'#39'-'#39'+SATarifarios.Codigo+'#39'-'#39'+a'#241'o AS cod,*,SATarifasDetalle' +
        '.Codigo AS cc,SATarifasDetalle.Tabla AS tb,SATarifasDetalle.A'#241'o ' +
        'AS tba'#241'o,SATarifarios.Tabla+'#39'-'#39'+SATarifarios.Codigo+'#39'-'#39'+a'#241'o+'#39' '#39'+' +
        'left(SATarifarios.Descripcion,70) AS Descr ,'
      
        '               dsala,msala,medesp,anest,medico_aux,SATarifasDeta' +
        'lle.puntaje as truvr, SATarifasDetalle.Valor as trvalor, SAServi' +
        'cios.Descripcion as desSer22'
      'FROM SAServicios,SAHomologacion,SATarifarios,SATarifasDetalle '
      'WHERE SAServicios.Tabla = SAHomologacion.Tabla1'
      '      AND SAServicios.Codigo = SAHomologacion.Codigo1'
      '      AND SAHomologacion.Tabla2 = SATarifarios.Tabla'
      '      AND SAHomologacion.Codigo2 = SATarifarios.Codigo'
      '      AND SATarifarios.Tabla = SATarifasDetalle.Tabla'
      '      AND SATarifarios.Codigo = SATarifasDetalle.Codigo'
      '      AND SAServicios.Tabla = :Tbl'
      '      AND SAServicios.Codigo = :Cod'
      '')
    Left = 136
    Top = 320
  end
  object ADOQueryConvenios: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Ase'
        Size = -1
        Value = Null
      end
      item
        Name = 'Cont'
        Size = -1
        Value = Null
      end
      item
        Name = 'tp'
        Size = -1
        Value = Null
      end
      item
        Name = 'cd'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM faconvenios'
      'WHERE Aseguradora=:Ase AND'
      '     No_Contrato = :Cont'
      '      AND Tp_PST=:tp'
      '      AND Cd_Pst=:cd')
    Left = 184
    Top = 264
  end
  object PopupMenu2: TPopupMenu
    Left = 3
    Top = 258
    object EliminarRegistro1: TMenuItem
      Caption = 'Eliminar Registro'
      OnClick = EliminarRegistro1Click
    end
    object CambiarlaCantidad1: TMenuItem
      Caption = 'Cambiar la Cantidad'
      OnClick = CambiarlaCantidad1Click
    end
  end
  object ADOQueryProd: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Ase'
        Size = -1
        Value = Null
      end
      item
        Name = 'Ncon'
        Size = -1
        Value = Null
      end
      item
        Name = 'Emp'
        Size = -1
        Value = Null
      end
      item
        Name = 'Area'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM INProductos  '
      
        'inner join INGrupo  on INProductos.cd_grupo = INGrupo.Codigo_Gru' +
        'po'
      
        'inner join FAConvenios on INProductos.CD_Producto   = FAConvenio' +
        's.CD_PST'
      
        'inner join CONTTerce on  FAConvenios.Aseguradora = (CONTTerce.Nu' +
        'mero_Documento+CONTTerce.Sucursal)'
      'left outer join INIVA on INGrupo.Codigo_IVA  = INIVA.Codigo_IVA'
      'WHERE   FAConvenios.TP_PST = '#39'P'#39' AND'
      '      FAConvenios.Aseguradora = :Ase AND'
      '      FAConvenios.No_Contrato = :Ncon AND   '
      '      INPRODUCTOS.ESTADO   = '#39'A'#39' AND'
      
        '      INProductos.CD_Producto IN (SELECT CD_PST FROM SACargosxAr' +
        'ea WHERE Tp_PST = '#39'P'#39
      '                               AND Empresa = :Emp'
      '                               AND Area = :Area)'
      'ORDER BY INProductos.Descripcion')
    Left = 769
    Top = 264
  end
  object QrytipoAdmision: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'Select *  '
      'from SATiposAdmision'
      'WHERE Estado = '#39'A'#39)
    Left = 360
    Top = 144
    object QrytipoAdmisionCD_TIPO_ADMISION: TStringField
      FieldName = 'CD_TIPO_ADMISION'
      Size = 2
    end
    object QrytipoAdmisionDESCRIPCION: TStringField
      FieldName = 'DESCRIPCION'
      Size = 50
    end
    object QrytipoAdmisionCD_PROD_CARGO: TStringField
      FieldName = 'CD_PROD_CARGO'
    end
  end
  object DataSource7: TDataSource
    DataSet = QrytipoAdmision
    Left = 329
    Top = 145
  end
  object DataSourceSVencim: TDataSource
    DataSet = ADOQuerySVencim
    Left = 11
    Top = 378
  end
  object ADOQuerySVencim: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Alma'
        Size = -1
        Value = Null
      end
      item
        Name = 'Produ'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT Lote,Fecha_Vencim,Left(lote+'#39'-'#39'+convert(varchar,Fecha_Ven' +
        'cim,103),25)  as llave, Sum(CANTIDAD) As Cantidad'
      'FROM INSaldosVencimiento'
      'WHERE Cd_Almacen = :Alma AND CD_Producto = :Produ'
      
        'GROUP BY Fecha_Vencim,Lote,Left(lote+'#39'-'#39'+convert(varchar,Fecha_V' +
        'encim,103),25)')
    Left = 43
    Top = 378
  end
  object DataSourceEncAgrupa: TDataSource
    DataSet = ADOQueryEncAgrupa
    Left = 339
    Top = 338
  end
  object ADOQueryEncAgrupa: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'ase'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 20
        Value = Null
      end
      item
        Name = 'cont'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 25
        Value = Null
      end>
    SQL.Strings = (
      'SELECT *, (valor * porcentaje)/100 as liq'
      'FROM   CRAgrupacion,FAConvenios'
      'WHERE  CRAgrupacion.Estado = '#39'A'#39' AND '
      '          FAConvenios.Aseguradora = :ase AND'
      '       FAConvenios.No_contrato = :cont AND   '
      '       FAConvenios.tp_pst = '#39'PAQ'#39' AND '
      '       CRAgrupacion.codigo = FAConvenios.cd_pst')
    Left = 387
    Top = 266
  end
  object ADOQueryDetAgrupa: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Cod'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CRDetAgrupacion'
      'WHERE Codigo = :Cod')
    Left = 291
    Top = 338
  end
  object ADOQueryUlt: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'cad'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT max(No_Cargo) as Ultimo'
      'FROM CREncCargos'
      'where Empresa_producto like :cad')
    Left = 105
    Top = 257
  end
  object DataSource5: TDataSource
    DataSet = ADOQuery4
    Left = 657
    Top = 385
  end
  object ADOQuery4: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Admin'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT CRDetCargos.No_Cargo,CRDetCargos.Sec_Cargo,Fecha,Registro' +
        ',No_admision,Aseguradora,Numero_Transaccion_Inventario,Id,Tp_Pst' +
        ',Cd_Pst,Descripcion,Unidades,Unidades_desc,Precio,Valor_Liquidad' +
        'o,Agrupacion,Lote,Fecha_Vence '
      'FROM CREncCargos,CRDetCargos'
      'WHERE CREncCargos.No_Cargo = CRDetCargos.No_Cargo AND'
      
        '              CREncCargos.No_Admision = :Admin AND CREncCargos.E' +
        'stado is null')
    Left = 729
    Top = 383
  end
  object QryG: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    Left = 461
    Top = 263
  end
  object QryInv: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    Left = 405
    Top = 391
  end
  object DataSource6: TDataSource
    DataSet = ADOTable4
    Left = 689
    Top = 161
  end
  object ADOTable4: TADOTable
    Connection = DataModule1.ADOConnection1
    Filtered = True
    TableName = 'CONTCENTROS'
    Left = 721
    Top = 161
  end
  object ADOQtmp: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'NOCARG'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CREncCargos'
      'WHERE No_Cargo = :NOCARG  AND Estado is NULL')
    Left = 841
    Top = 136
  end
  object ADOQtmp2: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'NOCARG'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CRDetCargos'
      'WHERE No_Cargo = :NOCARG'
      '')
    Left = 873
    Top = 136
  end
  object ADOTable1: TADOTable
    Connection = DataModule1.ADOConnection1
    TableName = 'CRParametros'
    Left = 129
    Top = 73
  end
  object ADOQueryServ: TADOQuery
    Connection = DataModule1.ADOConnection1
    CommandTimeout = 300
    Parameters = <
      item
        Name = 'Ase'
        Size = -1
        Value = Null
      end
      item
        Name = 'Ncon'
        Size = -1
        Value = Null
      end
      item
        Name = 'Emp'
        Size = -1
        Value = Null
      end
      item
        Name = 'Area'
        Size = -1
        Value = Null
      end
      item
        Name = 'tpadm'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT TABLA,CODIGO,SAServicios.DESCRIPCION,Capitulo,Tarifa,a'#241'o,' +
        'Porcentaje,SAServicios.Valor_Pesos'
      'FROM  SAServicios,SACapituloCups,INIVA,FAConvenios,CONTTerce'
      'WHERE SAServicios.Capitulo = SACapituloCups.Cod_Capitulo AND'
      '      SAServicios.Codigo = FAConvenios.CD_PST AND'
      '      SACapituloCups.Codigo_IVA = INIVA.Codigo_IVA AND '
      
        '      FAConvenios.Aseguradora = (CONTTerce.Numero_Documento+CONT' +
        'Terce.Sucursal) AND '
      '      FAConvenios.TP_PST = '#39'S'#39' AND'
      '      SAServicios.Estado = '#39'A'#39' AND       '
      '      FAConvenios.Aseguradora = :Ase AND'
      '      FAConvenios.No_Contrato = :Ncon AND  '
      '      SAServicios.Codigo IN (SELECT CD_PST '
      '                             FROM SACargosxArea '
      '                             WHERE Tp_PST = '#39'S'#39
      '                               AND Empresa =  :Emp'
      '                               AND Area =  :Area) AND '
      '      SAServicios.Codigo IN (SELECT Codigo_Servicio '
      '                             FROM SAServiciosxTpAdm '
      '                             WHERE Tp_Admision = :tpadm) AND'
      
        '      SAServicios.TABLA+SAServicios.CODIGO IN (SELECT SAHomologa' +
        'cion.Tabla1 + SAHomologacion.Codigo1'
      #9#9#9#9#9'FROM SAHomologacion,SATarifarios,SATarifasDetalle '
      #9#9#9#9#9'WHERE SAHomologacion.Tabla2 = SATarifarios.Tabla'
      #9#9#9#9#9'AND SAHomologacion.Codigo2 = SATarifarios.Codigo'
      #9#9#9#9#9'AND SATarifarios.Tabla = SATarifasDetalle.Tabla'
      #9#9#9#9#9'AND SATarifarios.Codigo = SATarifasDetalle.Codigo '
      #9#9#9#9#9'AND SATarifarios.TABLA = FAConvenios.Tarifa '
      #9#9#9#9#9'AND SATarifasDetalle.A'#241'o = FAConvenios.A'#241'o)'
      'ORDER BY SAServicios.Descripcion')
    Left = 776
    Top = 320
  end
  object ADOParamCargo: TADOTable
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    TableName = 'CRParametros'
    Left = 304
    Top = 8
  end
  object ADOTable2: TADOTable
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    TableName = 'INConsecFacturas'
    Left = 232
    Top = 8
  end
  object PopupMenu: TPopupMenu
    Left = 552
    Top = 232
    object EliminarCargo1: TMenuItem
      Caption = 'Anular Cargo'
      OnClick = EliminarCargo1Click
    end
    object Descargos1: TMenuItem
      Caption = 'Descargos'
      OnClick = Descargos1Click
    end
  end
end
