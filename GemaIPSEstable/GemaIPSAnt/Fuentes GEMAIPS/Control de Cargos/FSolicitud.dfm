object FSolicitaCargo: TFSolicitaCargo
  Left = 290
  Top = 182
  BorderStyle = bsSingle
  Caption = 'Solicitud para Realizar Cargos'
  ClientHeight = 498
  ClientWidth = 762
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -10
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 762
    Height = 498
    Align = alClient
    TabOrder = 0
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 760
      Height = 176
      Align = alTop
      TabOrder = 0
      object Label2: TLabel
        Left = 469
        Top = 11
        Width = 101
        Height = 13
        Caption = 'Numero Solicitud:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 456
        Top = 36
        Width = 114
        Height = 13
        Caption = 'Fecha del Solicitud:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 8
        Top = 131
        Width = 64
        Height = 13
        Caption = 'Ar'#233'a Solc.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 8
        Top = 155
        Width = 65
        Height = 13
        Caption = 'Ar'#233'a Proc.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label13: TLabel
        Left = 8
        Top = 8
        Width = 412
        Height = 36
        Caption = 'SOLICITUDES DE CARGOS'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -32
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object DateTimePicker1: TDateTimePicker
        Left = 576
        Top = 32
        Width = 177
        Height = 21
        Date = 38547.518292789350000000
        Time = 38547.518292789350000000
        TabOrder = 2
      end
      object Edit1: TEdit
        Left = 576
        Top = 8
        Width = 177
        Height = 21
        TabStop = False
        Color = cl3DLight
        ReadOnly = True
        TabOrder = 1
      end
      object DBLookupComboBox5: TDBLookupComboBox
        Left = 80
        Top = 152
        Width = 672
        Height = 21
        KeyField = 'Cod'
        ListField = 'Descr'
        ListSource = DataSource5
        TabOrder = 0
        TabStop = False
        OnExit = DBLookupComboBox5Exit
      end
      object GroupBox4: TGroupBox
        Left = 8
        Top = 56
        Width = 745
        Height = 70
        Caption = 'Admisi'#243'n:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
        object Label4: TLabel
          Left = 8
          Top = 47
          Width = 55
          Height = 13
          Caption = 'Paciente:'
        end
        object Label1: TLabel
          Left = 568
          Top = 24
          Width = 34
          Height = 13
          Caption = 'Edad:'
        end
        object Label7: TLabel
          Left = 376
          Top = 24
          Width = 56
          Height = 13
          Caption = 'Cama No:'
        end
        object Label12: TLabel
          Left = 184
          Top = 24
          Width = 83
          Height = 13
          Caption = 'Fec_Admisi'#243'n:'
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 8
          Top = 20
          Width = 169
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'NO_ADMISION'
          ListField = 'NO_ADMISION'
          ListSource = DataSource1
          ParentFont = False
          TabOrder = 2
        end
        object DBEdit1: TDBEdit
          Left = 272
          Top = 20
          Width = 97
          Height = 21
          TabStop = False
          DataField = 'FEC_ADMISION'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
        object DBEdit2: TDBEdit
          Left = 440
          Top = 20
          Width = 121
          Height = 21
          TabStop = False
          DataField = 'NO_CAMA'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
        end
        object DBEdit3: TDBEdit
          Left = 608
          Top = 20
          Width = 129
          Height = 21
          TabStop = False
          DataField = 'VALOR_EDAD'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 3
        end
        object DBEdit5: TDBEdit
          Left = 64
          Top = 44
          Width = 25
          Height = 21
          TabStop = False
          DataField = 'TP_IDENT_USUAR'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 4
        end
        object DBEdit6: TDBEdit
          Left = 88
          Top = 44
          Width = 89
          Height = 21
          TabStop = False
          DataField = 'NR_IDENT_USUAR'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 5
        end
        object DBEdit7: TDBEdit
          Left = 192
          Top = 44
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'PRIMER_APELLIDO'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 6
        end
        object DBEdit8: TDBEdit
          Left = 328
          Top = 44
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'SEGUNDO_APELLIDO'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 7
        end
        object DBEdit9: TDBEdit
          Left = 464
          Top = 44
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'PRIMER_NOMBRE'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 8
        end
        object DBEdit10: TDBEdit
          Left = 600
          Top = 44
          Width = 137
          Height = 21
          TabStop = False
          DataField = 'SEGUNDO_NOMBRE'
          DataSource = DataSource1
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 9
        end
      end
      object Edit2: TEdit
        Left = 80
        Top = 128
        Width = 673
        Height = 21
        Enabled = False
        TabOrder = 4
        Text = 'Edit2'
      end
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 177
      Width = 760
      Height = 320
      Align = alClient
      Caption = 'Detalle de la Solicitud:'
      TabOrder = 1
      object StringGrid1: TStringGrid
        Left = 2
        Top = 15
        Width = 756
        Height = 146
        Cursor = crHandPoint
        TabStop = False
        Align = alClient
        ColCount = 6
        Ctl3D = True
        RowCount = 2
        Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goColSizing, goRowSelect]
        ParentCtl3D = False
        PopupMenu = PopupMenu2
        TabOrder = 0
      end
      object GroupBox3: TGroupBox
        Left = 2
        Top = 161
        Width = 756
        Height = 157
        Align = alBottom
        Caption = 'Detalle de la Solicitud:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        object Label8: TLabel
          Left = 8
          Top = 18
          Width = 30
          Height = 13
          Caption = 'Tipo:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label9: TLabel
          Left = 51
          Top = 46
          Width = 55
          Height = 13
          Caption = 'Cantidad:'
          Visible = False
        end
        object Label23: TLabel
          Left = 8
          Top = 75
          Width = 52
          Height = 13
          Caption = 'Paquete:'
          Visible = False
        end
        object Shape1: TShape
          Left = 8
          Top = 66
          Width = 742
          Height = 1
        end
        object Label10: TLabel
          Left = 606
          Top = 75
          Width = 31
          Height = 13
          Caption = 'Cant:'
          Visible = False
        end
        object MaskEdit1: TMaskEdit
          Left = 115
          Top = 43
          Width = 105
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
          Text = ''
          Visible = False
          OnChange = MaskEdit2Change
          OnKeyPress = MaskEdit1KeyPress
        end
        object DBLookupComboBox3: TDBLookupComboBox
          Left = 115
          Top = 16
          Width = 113
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CD_PRODUCTO'
          ListField = 'CD_PRODUCTO'
          ListSource = DataSource3
          ParentFont = False
          TabOrder = 1
        end
        object DBLookupComboBox4: TDBLookupComboBox
          Left = 232
          Top = 16
          Width = 489
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CD_PRODUCTO'
          ListField = 'DESCRIPCION'
          ListSource = DataSource3
          ParentFont = False
          TabOrder = 2
          OnExit = DBLookupComboBox4Exit
        end
        object BitBtn1: TBitBtn
          Left = 725
          Top = 16
          Width = 25
          Height = 19
          Glyph.Data = {
            
            00333377737FFFFF773333303300000003333337337777777333}
          NumGlyphs = 2
          TabOrder = 5
          TabStop = False
          Visible = False
          OnClick = BitBtn1Click
        end
        object ComboBox1: TComboBox
          Left = 40
          Top = 16
          Width = 73
          Height = 21
          HelpType = htKeyword
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          OnChange = ComboBox1Change
          OnExit = ComboBox1Exit
          Items.Strings = (
            'P'
            'S'
            'PAQUETE'
            'O')
        end
        object DBLookupComboBox2: TDBLookupComboBox
          Left = 115
          Top = 16
          Width = 113
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CODIGO'
          ListField = 'CODIGO'
          ListSource = DataSource4
          ParentFont = False
          TabOrder = 6
        end
        object DBLookupComboBox6: TDBLookupComboBox
          Left = 232
          Top = 16
          Width = 489
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CODIGO'
          ListField = 'DESCRIPCION'
          ListSource = DataSource4
          ParentFont = False
          TabOrder = 7
          OnExit = DBLookupComboBox6Exit
        end
        object DBLookupComboBox7: TDBLookupComboBox
          Left = 115
          Top = 16
          Width = 113
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CODIGO'
          ListField = 'CODIGO'
          ListSource = DataSource6
          ParentFont = False
          TabOrder = 8
        end
        object DBLookupComboBox8: TDBLookupComboBox
          Left = 232
          Top = 16
          Width = 489
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'CODIGO'
          ListField = 'DESCRIPCION'
          ListSource = DataSource6
          ParentFont = False
          TabOrder = 9
          OnExit = DBLookupComboBox8Exit
        end
        object BitBtn2: TBitBtn
          Left = 256
          Top = 96
          Width = 161
          Height = 58
          Caption = 'Limpiar Grilla'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 10
          TabStop = False
          OnClick = BitBtn2Click
        end
        object BitBtn3: TBitBtn
          Left = 424
          Top = 96
          Width = 161
          Height = 58
          Caption = 'Adicionar a la Grilla'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 11
          OnClick = BitBtn3Click
        end
        object BitBtn4: TBitBtn
          Left = 592
          Top = 96
          Width = 161
          Height = 58
          Caption = 'Grabar Solicitud'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
          TabOrder = 12
          OnClick = BitBtn4Click
        end
        object DBLookupComboBox13: TDBLookupComboBox
          Left = 64
          Top = 72
          Width = 537
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'Codigo'
          ListField = 'Descripcion'
          ListSource = DataSource9
          ParentFont = False
          TabOrder = 13
          TabStop = False
          Visible = False
        end
        object MaskEdit2: TMaskEdit
          Left = 643
          Top = 72
          Width = 105
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 14
          Text = ''
          Visible = False
          OnChange = MaskEdit2Change
          OnKeyPress = MaskEdit1KeyPress
        end
        object Edit3: TEdit
          Left = 115
          Top = 16
          Width = 606
          Height = 21
          TabOrder = 15
        end
        object Edit4: TEdit
          Left = 226
          Top = 43
          Width = 527
          Height = 21
          TabOrder = 4
        end
      end
    end
  end
  object DataSource3: TDataSource
    DataSet = ADOQuery10
    OnDataChange = DataSource3DataChange
    Left = 681
    Top = 193
  end
  object ADOQuery2: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM CREncSolicitudes')
    Left = 657
    Top = 337
  end
  object ADOQuery3: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM CRDetSolicitudes')
    Left = 689
    Top = 337
  end
  object ADOTable1: TADOTable
    Connection = DataModule1.ADOConnection1
    TableName = 'CRParametros'
    Left = 673
    Top = 137
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 257
    Top = 1
  end
  object DataSource5: TDataSource
    DataSet = ADOQuery4
    Left = 377
    Top = 1
  end
  object ADOQuery1: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'adm'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT *,Primer_Apellido+'#39' '#39'+Segundo_Apellido+'#39' '#39'+Primer_Nombre+' +
        #39' '#39'+Segundo_Nombre as Nom '
      'FROM SAAdmisiones,SAPacientes '
      
        'WHERE (SAAdmisiones.Tp_Ident_Usuar = SAPacientes.Tp_Ident_Usuar ' +
        'AND'
      
        '               SAAdmisiones.Nr_Ident_Usuar = SAPacientes.Nr_Iden' +
        't_Usuar)  AND'
      '               (EST_ADMINSION = '#39'A'#39' OR EST_ADMINSION = '#39'C'#39')  AND'
      '               No_Admision = :adm'
      'ORDER BY No_Admision')
    Left = 289
    Top = 1
  end
  object ADOQuery4: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      
        'SELECT Comunes..SEAreas.Empresa+Comunes..SEAreas.Codigo as Cod,D' +
        'escripcion_Emp+'#39' - '#39'+Descripcion_Area as Descr ,*'
      'FROM Comunes..SEAreas,Comunes..SEEmpresas'
      'WHERE Comunes..SEAreas.Empresa = Comunes..SEEmpresas.Codigo AND'
      '      Area_Cargo='#39'S'#39' ')
    Left = 409
    Top = 1
  end
  object DataSource4: TDataSource
    DataSet = ADOQuery11
    OnDataChange = DataSource4DataChange
    Left = 681
    Top = 225
  end
  object DataSource6: TDataSource
    DataSet = ADOQuery12
    OnDataChange = DataSource6DataChange
    Left = 681
    Top = 257
  end
  object ADOQuery10: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Emp'
        Size = -1
        Value = Null
      end
      item
        Name = 'Area'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM INProductos,SACargosxArea'
      'WHERE Tp_PST = '#39'P'#39' AND CD_PST = CD_Producto'
      '  AND Empresa = :Emp'
      '  AND Area = :Area AND INPRODUCTOS.ESTADO = '#39'A'#39
      'ORDER BY INProductos.Descripcion')
    Left = 649
    Top = 192
  end
  object ADOQuery11: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Emp'
        Size = -1
        Value = Null
      end
      item
        Name = 'Area'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM SAServicios'
      'WHERE Estado = '#39'A'#39' AND '
      
        '              Codigo IN (SELECT CD_PST FROM SACargosxArea WHERE ' +
        'Tp_PST = '#39'S'#39
      '                               AND Empresa = :Emp'
      '                               AND Area = :Area)'
      'ORDER BY SAServicios.Descripcion')
    Left = 649
    Top = 223
  end
  object ADOQuery12: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Emp'
        Size = -1
        Value = Null
      end
      item
        Name = 'Area'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM SATarifarios,SACargosxArea'
      'WHERE Tp_PST = '#39'T'#39' AND CD_PST = Codigo'
      '  AND Empresa = :Emp'
      '  AND Area = :Area'
      'ORDER BY Codigo')
    Left = 649
    Top = 255
  end
  object PopupMenu2: TPopupMenu
    Left = 99
    Top = 466
    object EliminarRegistro1: TMenuItem
      Caption = 'Eliminar Registro'
      OnClick = EliminarRegistro1Click
    end
  end
  object ADOQuery14: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Cod'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CRDetAgrupacion'
      'WHERE Codigo = :Cod')
    Left = 3
    Top = 466
  end
  object DataSource9: TDataSource
    DataSet = ADOQuery13
    Left = 35
    Top = 466
  end
  object ADOQuery13: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      'SELECT * '
      'FROM CRAgrupacion')
    Left = 67
    Top = 466
  end
  object ADOQuery9: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Alma'
        Size = -1
        Value = Null
      end
      item
        Name = 'Produ'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT Lote,Fecha_Vencim,Left(lote+convert(varchar,Fecha_Vencim,' +
        '103),25)  as llave, Sum(CANTIDAD) As Cantidad'
      'FROM INSaldosVencimiento'
      'WHERE Cd_Almacen = :Alma AND CD_Producto = :Produ'
      
        'GROUP BY Fecha_Vencim,Lote,Left(lote+convert(varchar,Fecha_Venci' +
        'm,103),25)')
    Left = 35
    Top = 434
  end
  object DataSource8: TDataSource
    DataSet = ADOQuery9
    Left = 3
    Top = 434
  end
end
