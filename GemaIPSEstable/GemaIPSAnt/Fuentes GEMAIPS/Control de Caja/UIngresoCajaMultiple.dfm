object IngresosFacturaM: TIngresosFacturaM
  Left = 414
  Top = 114
  BorderStyle = bsSingle
  Caption = 'Ingresos por Multiples Facturas'
  ClientHeight = 598
  ClientWidth = 809
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -10
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  PopupMenu = PopupMenu1
  Position = poDefault
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 809
    Height = 598
    Align = alClient
    TabOrder = 0
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 807
      Height = 112
      Align = alTop
      TabOrder = 0
      object Label2: TLabel
        Left = 57
        Top = 57
        Width = 116
        Height = 13
        Caption = 'Numero Documento:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 568
        Top = 58
        Width = 40
        Height = 13
        Caption = 'Fecha:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 8
        Top = 8
        Width = 270
        Height = 32
        Caption = 'INGRESOS DE CAJA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -27
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label19: TLabel
        Left = 8
        Top = 88
        Width = 44
        Height = 13
        Caption = 'Cliente:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label16: TLabel
        Left = 424
        Top = 8
        Width = 69
        Height = 13
        Caption = 'Documento:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label18: TLabel
        Left = 424
        Top = 32
        Width = 61
        Height = 13
        Caption = 'Tipo Doc.:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DateTimePicker1: TDateTimePicker
        Left = 616
        Top = 54
        Width = 177
        Height = 21
        Date = 38547.518292789350000000
        Time = 38547.518292789350000000
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnChange = DateTimePicker1Change
      end
      object Edit1: TEdit
        Left = 196
        Top = 54
        Width = 177
        Height = 21
        TabStop = False
        Color = cl3DLight
        ReadOnly = True
        TabOrder = 6
      end
      object GroupBox2: TGroupBox
        Left = -112
        Top = -55
        Width = 785
        Height = 57
        Caption = 'Observaciones:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 5
        object Memo1: TMemo
          Left = 8
          Top = 16
          Width = 769
          Height = 33
          TabStop = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -10
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
      end
      object DBLookupComboBox2: TDBLookupComboBox
        Left = 56
        Top = 85
        Width = 137
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        KeyField = 'Numero_Documento'
        ListField = 'Numero_Documento'
        ListSource = DataSource1
        ParentFont = False
        TabOrder = 3
        OnExit = DBLookupComboBox2Exit
      end
      object DBLookupComboBox3: TDBLookupComboBox
        Left = 200
        Top = 85
        Width = 593
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        KeyField = 'Numero_Documento'
        ListField = 'Nombre'
        ListSource = DataSource1
        ParentFont = False
        TabOrder = 4
        OnExit = DBLookupComboBox3Exit
      end
      object DBLookupComboBox8: TDBLookupComboBox
        Left = 499
        Top = 2
        Width = 297
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        KeyField = 'Tipo_Documento'
        ListField = 'Descripcion'
        ListSource = DataSource3
        ParentFont = False
        TabOrder = 0
        OnExit = DBLookupComboBox2Exit
      end
      object DBLookupComboBox9: TDBLookupComboBox
        Left = 496
        Top = 29
        Width = 297
        Height = 21
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        KeyField = 'cod'
        ListField = 'Descripcion'
        ListSource = DataSource4
        ParentFont = False
        TabOrder = 1
        OnExit = DBLookupComboBox2Exit
      end
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 113
      Width = 807
      Height = 484
      Align = alClient
      Caption = 'Detalle de la Transacci'#243'n:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -10
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object StringGridCaja: TStringGrid
        Left = 2
        Top = 15
        Width = 803
        Height = 194
        Cursor = crHandPoint
        TabStop = False
        Align = alClient
        ColCount = 7
        Ctl3D = True
        RowCount = 2
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goColSizing, goRowSelect]
        ParentCtl3D = False
        ParentFont = False
        PopupMenu = PopupMenu1
        TabOrder = 0
        RowHeights = (
          24
          24)
      end
      object GroupBox3: TGroupBox
        Left = 2
        Top = 209
        Width = 803
        Height = 273
        Align = alBottom
        Caption = 'Detalle de la Transacci'#243'n:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -10
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        object SpeedButton2: TSpeedButton
          Left = 320
          Top = 214
          Width = 153
          Height = 55
          Caption = 'Borrar Grilla'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          OnClick = SpeedButton2Click
        end
        object SpeedButton1: TSpeedButton
          Left = 480
          Top = 214
          Width = 153
          Height = 55
          Caption = 'Adicionar a Grilla'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          OnClick = SpeedButton1Click
        end
        object SpeedButton3: TSpeedButton
          Left = 640
          Top = 214
          Width = 153
          Height = 55
          Caption = 'Grabar Trans.'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
          OnClick = SpeedButton3Click
        end
        object Label1: TLabel
          Left = 149
          Top = 30
          Width = 72
          Height = 13
          Caption = 'Factura No.:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 400
          Top = 31
          Width = 40
          Height = 13
          Caption = 'Fecha:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBText2: TDBText
          Left = 448
          Top = 31
          Width = 121
          Height = 17
          DataField = 'Fecha'
          DataSource = DataSource6
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label6: TLabel
          Left = 400
          Top = 15
          Width = 44
          Height = 13
          Caption = 'Cliente:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBText3: TDBText
          Left = 448
          Top = 15
          Width = 120
          Height = 17
          DataField = 'Cliente'
          DataSource = DataSource6
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object DBText1: TDBText
          Left = 656
          Top = 15
          Width = 137
          Height = 17
          DataField = 'Valor_Total'
          DataSource = DataSource6
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clFuchsia
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label8: TLabel
          Left = 584
          Top = 15
          Width = 54
          Height = 13
          Caption = 'Vr. Total:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label9: TLabel
          Left = 584
          Top = 35
          Width = 68
          Height = 13
          Caption = 'Vr. Pagado:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBText4: TDBText
          Left = 656
          Top = 35
          Width = 137
          Height = 17
          DataField = 'Valor_Pagado'
          DataSource = DataSource6
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label20: TLabel
          Left = 7
          Top = 30
          Width = 29
          Height = 13
          Caption = 'Emp:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 223
          Top = 27
          Width = 137
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'Numero_Factura'
          ListField = 'Numero_Factura;Empresa'
          ListSource = DataSource6
          ParentFont = False
          TabOrder = 1
        end
        object GroupBox4: TGroupBox
          Left = 10
          Top = 48
          Width = 783
          Height = 165
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 3
          object Shape1: TShape
            Left = 9
            Top = 42
            Width = 753
            Height = 79
            Brush.Color = clMenuBar
          end
          object Label7: TLabel
            Left = 16
            Top = 12
            Width = 133
            Height = 24
            Caption = 'Valor a Pagar:'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label10: TLabel
            Left = 24
            Top = 52
            Width = 59
            Height = 13
            Caption = 'Concepto:'
          end
          object Label13: TLabel
            Left = 520
            Top = 140
            Width = 61
            Height = 16
            Caption = 'Faltante:'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label17: TLabel
            Left = 16
            Top = 132
            Width = 130
            Height = 24
            Caption = 'Total Pagado:'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label11: TLabel
            Left = 88
            Top = 76
            Width = 99
            Height = 13
            Caption = 'Cuenta Contable:'
          end
          object Label12: TLabel
            Left = 552
            Top = 100
            Width = 34
            Height = 13
            Caption = 'Valor:'
          end
          object Label14: TLabel
            Left = 392
            Top = 76
            Width = 30
            Height = 13
            Caption = 'Tipo:'
          end
          object Label15: TLabel
            Left = 392
            Top = 12
            Width = 139
            Height = 24
            Caption = 'Total Facturas:'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object MaskEditVP: TMaskEdit
            Left = 152
            Top = 8
            Width = 225
            Height = 32
            BiDiMode = bdLeftToRight
            Color = clScrollBar
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentBiDiMode = False
            ParentFont = False
            TabOrder = 0
            Text = '0'
          end
          object MaskEditValor: TMaskEdit
            Left = 594
            Top = 96
            Width = 157
            Height = 21
            BiDiMode = bdLeftToRight
            ParentBiDiMode = False
            TabOrder = 6
            Text = '0'
            OnChange = MaskEditValorChange
            OnClick = MaskEditValorClick
            OnKeyPress = MaskEditValorKeyPress
          end
          object MaskEditFaltante: TMaskEdit
            Left = 592
            Top = 136
            Width = 159
            Height = 21
            Color = cl3DLight
            Enabled = False
            TabOrder = 8
            Text = '0'
          end
          object MaskEditTPagado: TMaskEdit
            Left = 168
            Top = 128
            Width = 217
            Height = 32
            Color = cl3DLight
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 7
            Text = '0'
          end
          object DBLookupComboBox4: TDBLookupComboBox
            Left = 88
            Top = 48
            Width = 137
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            KeyField = 'Concepto'
            ListField = 'Concepto'
            ListSource = DataSource2
            ParentFont = False
            TabOrder = 2
          end
          object DBLookupComboBox5: TDBLookupComboBox
            Left = 225
            Top = 48
            Width = 528
            Height = 21
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            KeyField = 'Concepto'
            ListField = 'Descripcion'
            ListSource = DataSource2
            ParentFont = False
            TabOrder = 3
          end
          object DBLookupComboBox6: TDBLookupComboBox
            Left = 225
            Top = 72
            Width = 160
            Height = 21
            Color = cl3DLight
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            KeyField = 'Concepto'
            ListField = 'Cuenta'
            ListSource = DataSource2
            ParentFont = False
            TabOrder = 4
          end
          object DBLookupComboBox7: TDBLookupComboBox
            Left = 433
            Top = 72
            Width = 160
            Height = 21
            Color = cl3DLight
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            KeyField = 'Concepto'
            ListField = 'Tipo'
            ListSource = DataSource2
            ParentFont = False
            TabOrder = 5
          end
          object MaskEditTF: TMaskEdit
            Left = 536
            Top = 8
            Width = 225
            Height = 32
            BiDiMode = bdLeftToRight
            Color = clScrollBar
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -19
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentBiDiMode = False
            ParentFont = False
            TabOrder = 1
            Text = '0'
          end
        end
        object CheckBox1: TCheckBox
          Left = 223
          Top = 9
          Width = 94
          Height = 17
          TabStop = False
          Caption = 'Sin Factura'
          TabOrder = 2
          OnClick = CheckBox1Click
        end
        object DBLookupComboBox10: TDBLookupComboBox
          Left = 39
          Top = 27
          Width = 98
          Height = 21
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'Codigo'
          ListField = 'Codigo;Descripcion_emp'
          ListSource = DataSource5
          ParentFont = False
          TabOrder = 0
          OnExit = DBLookupComboBox10Exit
        end
      end
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 3
    Top = 522
    object EliminarRegistro1: TMenuItem
      Caption = 'Eliminar Registro'
      OnClick = EliminarRegistro1Click
    end
  end
  object ADOTable3: TADOTable
    Connection = DataModule1.ADOConnection1
    TableName = 'CAParametros'
    Left = 585
    Top = 137
  end
  object ADOQuery2: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'emp'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CADetFacturas'
      'WHERE Left(Numero_Ingreso,1) = :emp')
    Left = 465
    Top = 137
  end
  object ADOQuery1: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'emp'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CAIngresos'
      'WHERE Left(Numero_Ingreso,1) = :emp')
    Left = 651
    Top = 136
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery3
    Left = 232
    Top = 16
  end
  object ADOQuery3: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'usu'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 50
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT DISTINCT CONTTerce.Numero_Documento + CONTTerce.Sucursal ' +
        'as Numero_Documento,CONTTerce.Primer_Apellido + '#39' '#39' + CONTTerce.' +
        'Primer_Nombre as Nombre,CONTTerce.sucursal'
      'FROM'#9'CONTTerce left outer join INEncFacturacion on'
      #9#9'CONTTerce.Numero_Documento = INEncFacturacion.Cliente '
      'WHERE'#9'CONTTerce.Numero_Documento is not null and '
      '        CONTTerce.Numero_Documento+CONTTerce.sucursal =  :usu')
    Left = 301
    Top = 16
  end
  object DataSource2: TDataSource
    DataSet = ADOQueryConceptos
    OnDataChange = DataSource2DataChange
    Left = 472
    Top = 216
  end
  object ADOQueryConceptos: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      'SELECT *'
      'FROM CAConceptos'
      'WHERE Estado = '#39'A'#39
      'ORDER BY Concepto')
    Left = 589
    Top = 216
  end
  object DataSource3: TDataSource
    DataSet = ADOQuery4
    Left = 472
    Top = 8
  end
  object ADOQuery4: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      'SELECT *'
      'FROM CADocumentos'
      'WHERE Estado = '#39'A'#39)
    Left = 541
    Top = 8
  end
  object DataSource4: TDataSource
    DataSet = ADOQuery5
    Left = 472
    Top = 56
  end
  object ADOQuery5: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'doc'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT documento_caja+Tipo_Documento_Caja as cod,*'
      'FROM CATiposDocumentos'
      'WHERE Documento_Caja = :doc AND Estado = '#39'A'#39)
    Left = 541
    Top = 56
  end
  object DataSource6: TDataSource
    DataSet = ADOQueryEncFacturas
    OnDataChange = DataSource6DataChange
    Left = 32
    Top = 256
  end
  object ADOQueryEncFacturas: TADOQuery
    Connection = DataModule1.ADOConnection1
    CommandTimeout = 90
    Parameters = <
      item
        Name = 'cl'
        Size = -1
        Value = Null
      end
      item
        Name = 'em'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT INENCFacturacion.Empresa+INENCFacturacion.Numero_Factura ' +
        'as cod,INENCFacturacion.Empresa,INENCFacturacion.Numero_Factura,' +
        'INENCFacturacion.Fecha,Cliente,INENCFacturacion.Estado,'
      
        '       Pagada_Total,max(INENCFacturacion.Valor_Total) as Valor_S' +
        'ubtotal,max(INENCFacturacion.Valor_IVA) as Valor_IVA,'
      '       max(INENCFacturacion.Valor_Descuento) as Valor_Descuento,'
      
        '       max(INENCFacturacion.Valor_Total+INENCFacturacion.Valor_I' +
        'VA-INENCFacturacion.Valor_Descuento) as Valor_Total,'
      '       sum(CADetFacturas.Valor) as Valor_Pagado'
      'FROM   INENCFacturacion left outer join CADetFacturas on '
      #9#9'    (INENCFacturacion.Empresa = CADetFacturas.Empresa AND'
      
        #9#9#9' INENCFacturacion.Numero_Factura = CADetFacturas.Numero_Factu' +
        'ra) '
      'WHERE       (estado <> '#39'A'#39' or estado is null) AND '
      
        '       CADetFacturas.Numero_Ingreso IN (SELECT Numero_Ingreso FR' +
        'OM CAIngresos WHERE Estado IS null) AND'
      '       Cliente = left(:cl,len(cliente)) AND'
      '       INENCFacturacion.Empresa = :em'
      
        'GROUP BY INENCFacturacion.Empresa+INENCFacturacion.Numero_Factur' +
        'a,INENCFacturacion.Empresa,INENCFacturacion.Numero_Factura,INENC' +
        'Facturacion.Fecha,Cliente,INENCFacturacion.Estado,Pagada_Total'
      
        'HAVING  max(INENCFacturacion.Valor_Total+INENCFacturacion.Valor_' +
        'IVA-INENCFacturacion.Valor_Descuento)  <> sum(CADetFacturas.Valo' +
        'r) OR'
      '        sum(CADetFacturas.Valor) is null')
    Left = 125
    Top = 256
  end
  object DataSource5: TDataSource
    DataSet = ADOQuery6
    Left = 32
    Top = 192
  end
  object ADOQuery6: TADOQuery
    Connection = DataModule1.ADOComunes
    CommandTimeout = 90
    Parameters = <>
    SQL.Strings = (
      'SELECT Codigo,Descripcion_Emp,Nit,BaseDatos'
      'FROM  SEEmpresas')
    Left = 125
    Top = 192
  end
  object ADOConnection1: TADOConnection
    LoginPrompt = False
    Provider = 'SQLOLEDB.1'
    Left = 16
    Top = 8
  end
  object QryApoyo: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    Left = 721
    Top = 137
  end
  object ADOQCierres: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <
      item
        Name = 'Per'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CONTCierres'
      'WHERE Periodo_Proceso = :Per')
    Left = 97
    Top = 9
  end
end
