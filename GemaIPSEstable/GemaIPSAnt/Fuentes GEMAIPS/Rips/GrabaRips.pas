...
procedure TFormGrabaRIPS.JvBitBtn2Click(Sender: TObject);
var i : Integer;
begin
  QryApoyo.Close;
  QryApoyo.SQL.Clear;
...
procedure TFormGrabaRIPS.LimpiaVector;
var i : integer;
begin
...
procedure TFormGrabaRIPS.JvBitBtn1Click(Sender: TObject);
var i : Integer;
begin
  QryGeneral.Close;
  QryGeneral.SQL.Clear;
...
procedure TFormGrabaRIPS.BtnDesdeClick(Sender: TObject);
var i, e : Integer;
begin
  PnlHasta.Height := 0;
...
procedure TFormGrabaRIPS.LimpiaGrilla(Grilla : TJvStringGrid; Col : Integer);
var i,e : Integer;
begin
  i := 1;
...
procedure TFormGrabaRIPS.ChkListaClick(Sender: TObject);
var i : Integer;
begin
...
procedure TFormGrabaRIPS.SGDesdeDblClick(Sender: TObject);
var i : Integer;
begin
  i := 0;
  while i < Length(vecFacturas) do
...
procedure TFormGrabaRIPS.BtnHastaClick(Sender: TObject);
var i, e    : Integer;
    MyRect  : TGridRect;
    m       : Integer;
begin
...
procedure TFormGrabaRIPS.SGHastaDblClick(Sender: TObject);
begin
  LblHasta.Caption           := SGHasta.Rows[SGHasta.Row].Strings[1];
  PnlHasta.Height            := 0;
end;

procedure TFormGrabaRIPS.BtnCargarClick(Sender: TObject);
   ...
   begin
     ...
     Begin
       ...
       begin
         ...
         //PAQUETE ADICIONADO POR FREDDY BUENO 18/07/2013
         //Se debe adicionar esta rutina en productos despues de ser probada
         //primero con muchos paquetes en servicios
         Else Begin
           if QryGeneral.Fields.FieldByName('Tipo_PST').AsString = 'G' then
           Begin
             ADOQPaquete.Close;
             ADOQPaquete.SQL.Clear;
             ADOQPaquete.SQL.Add('select * from INDetFacturacion,CRDetCargos');
             ADOQPaquete.SQL.Add('where  numero_Factura = '+chr(39)+vecFacXCliente[i,0]+chr(39));
             ADOQPaquete.SQL.Add(' AND INDetFacturacion.No_Cargo = CRDetCargos.No_Cargo ');
             ADOQPaquete.SQL.Add(' AND INDetFacturacion.Nombre   = CRDetCargos.Agrupacion ');
             ADOQPaquete.SQL.Add(' AND INDetFacturacion.Empresa = '+chr(13)+Edit1.Text+chr(13));
             ADOQPaquete.Open;


             while not ADOQPaquete.Eof do
             begin
               QryApoyo.Close;
               QryApoyo.SQL.Clear;
               QryApoyo.SQL.Add('select * from SAAdmisiones, SAPacientes');
               ...
               begin
                 ...
                 SGAc.Cells[4, SGAc.RowCount-2]  := FormatDateTime('DD/MM/YYYY',QryApoyo.Fields.FieldByName('FEC_ADMISION').AsDateTime);
                 SGAc.Cells[5, SGAc.RowCount-2]  := QryApoyo.Fields.FieldByName('NO_AUTORIZACION').AsString;
                 SGAc.Cells[6, SGAc.RowCount-2]  := Trim(copy(ADOQPaquete.Fields.FieldByName('Cd_PST').AsString,1,8));
                 SGAc.Cells[7, SGAc.RowCount-2]  := QryApoyo.Fields.FieldByName('FINALIDAD_CONS').AsString;
                 SGAc.Cells[8, SGAc.RowCount-2]  := QryApoyo.Fields.FieldByName('CAUSA_EXTERNA').AsString;
                 SGAc.Cells[9, SGAc.RowCount-2]  := QryApoyo.Fields.FieldByName('DIAGNOSTICO').AsString;
                 SGAc.Cells[10, SGAc.RowCount-2] := QryApoyo.Fields.FieldByName('DIAGN_I_REL1').AsString;
                 ...
                 SGAc.Cells[13, SGAc.RowCount-2] := QryApoyo.Fields.FieldByName('TP_DIAGN_PPAL').AsString;
                 //SGAc.Cells[14, SGAc.RowCount-2] := QryGeneral.Fields.FieldByName('Valor_Unitario').AsString;
                 SGAc.Cells[14, SGAc.RowCount-2] := FloatToStr((ADOQPaquete.Fields.FieldByName('Valor_Unitario').AsFloat));
                 SGAc.Cells[15, SGAc.RowCount-2] := '0';
                 SGAc.Cells[16, SGAc.RowCount-2] := FloatToStr((ADOQPaquete.Fields.FieldByName('Valor_Total').AsFloat));

                 ValorAC := ValorAC + (ADOQPaquete.Fields.FieldByName('Valor_Total').AsFloat);
               end;
               ...
               begin
                 ...
                 while e < Can do
                   begin
                     ...
                     SGAp.Cells[4,SGAp.RowCount -2]   := FormatDateTime('DD/MM/YYYY',QryApoyo.Fields.FieldByName('FEC_ADMISION').AsDateTime);
                     SGAp.Cells[5,SGAp.RowCount -2]   := QryApoyo.Fields.FieldByName('NO_AUTORIZACION').AsString;
                     SGAp.Cells[6,SGAp.RowCount -2]   := Trim(copy(ADOQPaquete.Fields.FieldByName('Cd_PST').AsString,1,8));
                     SGAp.Cells[7,SGAp.RowCount -2]   := QryApoyo.Fields.FieldByName('PROC_AMBITO').AsString;
                     SGAp.Cells[8,SGAp.RowCount -2]   := QryApoyo.Fields.FieldByName('PROC_FINALIDAD').AsString;
                     SGAp.Cells[9,SGAp.RowCount -2]   := QryApoyo.Fields.FieldByName('PROC_ATIENDE').AsString;
                     if QryApoyo1.FieldValues['Quirurgico_o_parto'] = 'S' then
                     ...
                     SGAp.Cells[12,SGAp.RowCount -2]  := QryApoyo.Fields.FieldByName('DIAGN_COMPL').AsString;
                     SGAp.Cells[13,SGAp.RowCount -2]  := QryApoyo.Fields.FieldByName('PROC_FORMA').AsString;
                     SGAp.Cells[14,SGAp.RowCount -2]  := FloatToStr((ADOQPaquete.Fields.FieldByName('Valor_Unitario').AsFloat));
                     inc(e);
                   end;
               end;   //si el tipo de admision es P
               ...
               begin
                 ...
                 SGAt.Cells[4,SGAt.RowCount-2]   := QryApoyo.Fields.FieldByName('NO_AUTORIZACION').AsString;
                 SGAt.Cells[5,SGAt.RowCount-2]   := QryApoyo1.Fields.FieldByName('Tipo').AsString;
                 SGAt.Cells[6,SGAt.RowCount-2]   := trim(copy(ADOQPaquete.Fields.FieldByName('Cd_PST').AsString,1,20));
                 SGAt.Cells[7,SGAt.RowCount-2]   := trim(copy(QryApoyo1.Fields.FieldByName('DESCRIPCION').AsString,1,60));
                 SGAt.Cells[8,SGAt.RowCount-2]   := QryGeneral.Fields.FieldByName('Cantidad').AsString;
                 SGAt.Cells[9,SGAt.RowCount-2]   := FloatToStr((ADOQPaquete.Fields.FieldByName('Valor_Unitario').AsFloat));
                 SGAt.Cells[10,SGAt.RowCount-2]  := FloatToStr((ADOQPaquete.Fields.FieldByName('Cantidad').AsInteger * QryGeneral.Fields.FieldByName('Valor_Unitario').AsFloat));
               end;

               ADOQPaquete.Next;
             End;  //Final While Paquette
           End;  // Final Pregunta si es Paquette
         End; //Final Else del paquete
         QryGeneral.Next;
       end;
     End;
     inc(i);
   end;  //Final While Servicios
...
procedure TFormGrabaRIPS.FormCreate(Sender: TObject);
begin
  SGCt.ColCount := 4;
  SGCt.RowCount := 2;
...
procedure TFormGrabaRIPS.BtnGenerarClick(Sender: TObject);
var F: TextFile;
    i,j:Integer;
    Path:String;
begin
  Path:= drive.Drive + ':\RIPS\'+trim(EditCodigoEPS.Text)+'_'+trim(EditNombre.Text)+'\R'+trim(JvEdit2.Text)+'\';
  ...
  Showmessage('Los RIPS fueron Generados en '+Path);
end;

procedure TFormGrabaRIPS.FormShow(Sender: TObject);
begin
  Edit1.Text := DataModule1.ADOQuery1.Fields.fieldByName('Empresa_Trabaja').AsString;
end;

procedure TFormGrabaRIPS.Button1Click(Sender: TObject);
begin
  if ActiveMDIChild = nil then  ActiveMDIChild.Close
end;

procedure TFormGrabaRIPS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := cafree;
  FormGrabaRIPS := nil;
end;
