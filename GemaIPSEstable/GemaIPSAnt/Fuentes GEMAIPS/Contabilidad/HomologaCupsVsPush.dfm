object HomologaCupPush: THomologaCupPush
  Left = 237
  Top = 78
  BorderStyle = bsSingle
  Caption = 'Homologaci'#243'n de Cups vs Cuentas Contables'
  ClientHeight = 602
  ClientWidth = 868
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 81
    Width = 868
    Height = 521
    Align = alClient
    TabOrder = 0
    object GroupBox1: TGroupBox
      Left = 1
      Top = 233
      Width = 336
      Height = 287
      Align = alClient
      Caption = 'Servicios:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      object DBLookupListBox1: TDBLookupListBox
        Left = 2
        Top = 15
        Width = 332
        Height = 264
        Align = alClient
        KeyField = 'DESCRIPCION'
        ListField = 'Codigo;DESCRIPCION'
        ListSource = DataSource4
        TabOrder = 0
      end
    end
    object Panel2: TPanel
      Left = 337
      Top = 233
      Width = 73
      Height = 287
      Align = alRight
      TabOrder = 2
      object SpeedButton1: TSpeedButton
        Left = 8
        Top = 64
        Width = 57
        Height = 25
        Glyph.Data = {
          
          3333333333333333333333333333333333333333333333333333}
        NumGlyphs = 2
        OnClick = SpeedButton1Click
      end
      object SpeedButton2: TSpeedButton
        Left = 8
        Top = 96
        Width = 57
        Height = 25
        Glyph.Data = {
          
          33333777333333333333CC333333333333337733333333333333}
        NumGlyphs = 2
        OnClick = SpeedButton2Click
      end
      object SpeedButton3: TSpeedButton
        Left = 8
        Top = 128
        Width = 57
        Height = 25
        Glyph.Data = {
          
          CCC333333333333777FF33333333333333CC3333333333333773}
        NumGlyphs = 2
        OnClick = SpeedButton3Click
      end
      object SpeedButton4: TSpeedButton
        Left = 8
        Top = 160
        Width = 57
        Height = 25
        Glyph.Data = {
          
          3333333333333333333333333333333333333333333333333333}
        NumGlyphs = 2
        OnClick = SpeedButton4Click
      end
    end
    object GroupBox2: TGroupBox
      Left = 410
      Top = 233
      Width = 457
      Height = 287
      Align = alRight
      Caption = 'Cuentas PUSH:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      object DBLookupListBox2: TDBLookupListBox
        Left = 2
        Top = 57
        Width = 453
        Height = 225
        Align = alClient
        KeyField = 'Codigo'
        ListField = 'Codigo;Descripcion'
        ListSource = DataSource5
        TabOrder = 0
      end
      object Panel5: TPanel
        Left = 2
        Top = 15
        Width = 453
        Height = 42
        Align = alTop
        TabOrder = 1
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 8
          Top = 8
          Width = 353
          Height = 21
          KeyField = 'Cuenta'
          ListField = 'Descripcion;Cuenta'
          ListSource = DataSource1
          TabOrder = 0
        end
        object DBEdit1: TDBEdit
          Left = 360
          Top = 8
          Width = 89
          Height = 21
          DataField = 'Cuenta'
          DataSource = DataSource1
          ReadOnly = True
          TabOrder = 1
        end
      end
    end
    object Panel3: TPanel
      Left = 1
      Top = 1
      Width = 866
      Height = 232
      Align = alTop
      TabOrder = 0
      object Label2: TLabel
        Left = 8
        Top = 10
        Width = 30
        Height = 13
        Caption = 'Tipo:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object GroupBox4: TGroupBox
        Left = 8
        Top = 29
        Width = 849
        Height = 137
        Caption = 'Agrupaci'#243'n de Servicios:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        Visible = False
        object Label9: TLabel
          Left = 16
          Top = 19
          Width = 51
          Height = 13
          Caption = 'Capitulo:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label10: TLabel
          Left = 18
          Top = 43
          Width = 48
          Height = 13
          Caption = 'Articulo:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label11: TLabel
          Left = 448
          Top = 19
          Width = 45
          Height = 13
          Caption = 'Titulo I:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label12: TLabel
          Left = 448
          Top = 43
          Width = 49
          Height = 13
          Caption = 'Titulo II:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBLookupComboBox3: TDBLookupComboBox
          Left = 72
          Top = 16
          Width = 361
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'COD_CAPITULO'
          ListField = 'DESCRIPCION'
          ListSource = DataSource3
          ParentFont = False
          TabOrder = 0
          OnClick = DBLookupComboBox3Click
        end
        object DBLookupComboBox4: TDBLookupComboBox
          Left = 72
          Top = 40
          Width = 361
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'COD_ARTICULO'
          ListField = 'DESCRIPCION'
          ListSource = DataSource6
          ParentFont = False
          TabOrder = 1
          OnClick = DBLookupComboBox4Click
        end
        object DBLookupComboBox5: TDBLookupComboBox
          Left = 496
          Top = 16
          Width = 345
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'COD_TITULO1'
          ListField = 'DESCRIPCION'
          ListSource = DataSource7
          ParentFont = False
          TabOrder = 2
          OnClick = DBLookupComboBox5Click
        end
        object DBLookupComboBox6: TDBLookupComboBox
          Left = 496
          Top = 40
          Width = 345
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'COD_TITULO2'
          ListField = 'DESCRIPCION'
          ListSource = DataSource8
          ParentFont = False
          TabOrder = 3
        end
      end
      object ComboBox1: TComboBox
        Left = 72
        Top = 5
        Width = 65
        Height = 21
        Enabled = False
        TabOrder = 0
        Text = 'S'
        OnChange = ComboBox1Change
        Items.Strings = (
          'S')
      end
      object GroupBox3: TGroupBox
        Left = 8
        Top = 176
        Width = 849
        Height = 49
        Caption = 'Filtros:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        object Label6: TLabel
          Left = 16
          Top = 16
          Width = 93
          Height = 13
          Caption = 'Palabra Incluye:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label7: TLabel
          Left = 624
          Top = 24
          Width = 96
          Height = 13
          Caption = 'Palabra Excluye:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          Visible = False
        end
        object Label1: TLabel
          Left = 232
          Top = 16
          Width = 89
          Height = 13
          Caption = 'C'#243'digo Incluye:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Edit1: TEdit
          Left = 112
          Top = 13
          Width = 105
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          OnChange = Edit1Change
        end
        object Edit2: TEdit
          Left = 731
          Top = 21
          Width = 105
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          Visible = False
          OnChange = Edit1Change
        end
        object Edit3: TEdit
          Left = 328
          Top = 13
          Width = 105
          Height = 21
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnChange = Edit3Change
        end
      end
      object BitBtn3: TBitBtn
        Left = 672
        Top = 104
        Width = 177
        Height = 57
        Caption = 'Ejecutar Selecci'#243'n'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
        TabOrder = 3
        OnClick = BitBtn3Click
      end
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 0
    Width = 868
    Height = 81
    Align = alTop
    TabOrder = 1
    object BitBtn1: TBitBtn
      Left = 5
      Top = 6
      Width = 84
      Height = 67
      Hint = 'Consulta de la Tabla de Servicios'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFF00}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      TabStop = False
      OnClick = BitBtn1Click
    end
  end
  object DataSource4: TDataSource
    DataSet = ADOQuery1
    Left = 150
    Top = 294
  end
  object DataSource5: TDataSource
    DataSet = ADOQuery5
    Left = 409
    Top = 529
  end
  object ADOQuery1: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      'SELECT * ,SAServicios.descripcion as descr'
      'FROM SAServicios'
      
        'WHERE (SAServicios.Cuenta NOT IN (SELECT CONTCuenta.Cuenta FROM ' +
        'CONTCuenta WHERE CONTCuenta.Estado = '#39'A'#39') OR'
      '      SAServicios.Cuenta = '#39#39') AND'
      '         SAServicios.Estado = '#39'A'#39)
    Left = 181
    Top = 294
  end
  object DataSource3: TDataSource
    DataSet = ADOTable2
    Left = 898
    Top = 65
  end
  object DataSource6: TDataSource
    DataSet = ADOTable3
    Left = 898
    Top = 97
  end
  object DataSource7: TDataSource
    DataSet = ADOTable4
    Left = 898
    Top = 129
  end
  object DataSource8: TDataSource
    DataSet = ADOTable6
    Left = 898
    Top = 161
  end
  object ADOTable2: TADOTable
    Connection = DataModule1.ADOConnection1
    TableName = 'SACapituloCups'
    Left = 929
    Top = 65
  end
  object ADOTable3: TADOTable
    Connection = DataModule1.ADOConnection1
    IndexFieldNames = 'COD_CAPITULO'
    MasterFields = 'COD_CAPITULO'
    MasterSource = DataSource3
    TableName = 'SAArticulosCups'
    Left = 929
    Top = 97
  end
  object ADOTable4: TADOTable
    Connection = DataModule1.ADOConnection1
    IndexFieldNames = 'COD_CAPITULO;COD_ARTICULO'
    MasterFields = 'COD_CAPITULO;COD_ARTICULO'
    MasterSource = DataSource6
    TableName = 'SATituloICups'
    Left = 929
    Top = 129
  end
  object ADOTable6: TADOTable
    Connection = DataModule1.ADOConnection1
    IndexFieldNames = 'COD_CAPITULO;COD_ARTICULO;COD_TITULO1'
    MasterFields = 'COD_CAPITULO;COD_ARTICULO;COD_TITULO1'
    MasterSource = DataSource7
    TableName = 'SATituloIICups'
    Left = 929
    Top = 161
  end
  object DataSource9: TDataSource
    DataSet = ADOTable7
    Left = 898
    Top = 193
  end
  object DataSource10: TDataSource
    DataSet = ADOTable8
    Left = 898
    Top = 225
  end
  object ADOTable7: TADOTable
    Connection = DataModule1.ADOConnection1
    TableName = 'INGrupo'
    Left = 929
    Top = 193
  end
  object ADOTable8: TADOTable
    Connection = DataModule1.ADOConnection1
    IndexFieldNames = 'Codigo_Grupo'
    MasterFields = 'Codigo_Grupo'
    MasterSource = DataSource9
    TableName = 'INSubgrupo'
    Left = 930
    Top = 225
  end
  object DataSource11: TDataSource
    DataSet = ADOQuery3
    Left = 898
    Top = 257
  end
  object ADOQuery3: TADOQuery
    Connection = DataModule1.ADOConnection1
    Parameters = <>
    SQL.Strings = (
      'SELECT distinct tabla FROM SATarifarios')
    Left = 929
    Top = 257
  end
  object ADOQuery5: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'cta'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM SAServicios'
      'WHERE Cuenta = :cta')
    Left = 441
    Top = 529
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery2
    OnDataChange = DataSource1DataChange
    Left = 785
    Top = 361
  end
  object ADOQuery2: TADOQuery
    Connection = DataModule1.ADOConnection1
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'Emp'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 4
        Value = Null
      end
      item
        Name = 'Area'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 5
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * '
      'FROM CONTCuenta '
      'WHERE left(CONTCuenta.Cuenta,1) = '#39'4'#39' AND ESTADO = '#39'A'#39)
    Left = 817
    Top = 361
  end
end
